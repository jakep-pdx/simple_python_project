""" This module determines next sematic version to use when building a package """


def get_cur_version_from_file_name(file_name):
    """ return the next semantic version"""
    print("file name provided:", file_name)
    cur_ver = None
    try:
...
def get_version_given_existing_version(cur_ver, release_type):
    """ return the next semantic version """
    print("current version provided:", cur_ver)
    print("release type provided:", release_type)
    major = None
    minor = None
    patch = None
    try:
        ...
        raise ValueError("unable to break down current version as provided:", cur_ver) from exc
    new_ver = None
...
def get_next_version_given_file_name(file_name, release_type):
    """ get the current version from a file name, then determine next version for release type """
